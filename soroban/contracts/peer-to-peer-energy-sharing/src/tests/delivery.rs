...
fn test_delivery_validation_failures() {
    ...
    let result = setup.client.try_deliver_energy(
        &agreement_id, &100u64, &1500u64, &setup.consumer
    );
    assert_eq!(result, Err(Ok(SharingError::NotAuthorized)));
    
    // Excessive energy delivery
    let result = setup.client.try_deliver_energy(
        &agreement_id, &150u64, &2250u64, &setup.provider
    );
    assert_eq!(result, Err(Ok(SharingError::InsufficientEnergy)));
    
    // Non-existent agreement
    let result = setup.client.try_deliver_energy(
        &999u64, &100u64, &1500u64, &setup.provider
    );
...
fn test_partial_delivery_with_meter_verification() {
    ...
    let transaction_id = setup.client.deliver_energy(
        &agreement_id, &75u64, &meter_reading, &setup.provider
    );
