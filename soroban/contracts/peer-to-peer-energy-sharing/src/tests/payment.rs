...
fn test_successful_payment_settlement() {
    ...
    let transaction_id = setup.client.deliver_energy(
        &agreement_id, &100u64, &1500u64, &setup.provider
    );
    ...
    // Settle payment
    setup.client.settle_payment(&transaction_id, &setup.consumer);
...
fn test_payment_authorization_failures() {
    ...
    let result = setup.client.try_settle_payment(&transaction_id, &setup.prosumer3);
    ...
    let result = setup.client.try_settle_payment(&transaction_id, &setup.consumer);
...
fn test_payment_accuracy_partial_delivery() {
    ...
    let agreement_id = setup.create_custom_agreement(
        &setup.provider, &setup.consumer, 250, 120, setup.get_future_deadline()
    );
    
    // Partial delivery: 200 out of 250 kWh at 120 per kWh
    let transaction_id = setup.client.deliver_energy(
        &agreement_id, &200u64, &3000u64, &setup.provider
    );
    
    let consumer_balance_before = get_token_balance(&setup.env, &setup.token_contract, &setup.consumer);
    let provider_balance_before = get_token_balance(&setup.env, &setup.token_contract, &setup.provider);
    
    setup.client.settle_payment(&transaction_id, &setup.provider);
...
fn test_transaction_history_tracking() {
    ...
    assert_eq!(provider_history.get(0).unwrap().transaction_id, transaction_id);
    assert_eq!(consumer_history.get(0).unwrap().transaction_id, transaction_id);
}